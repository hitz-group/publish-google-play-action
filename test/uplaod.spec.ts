...
describe("Test all methods in the upload class", () => {
  ...
  let OriginalUploadObj = new upload(
    jwtObjOriginal,
    editOptionsObj,
    releaseFile
  );
  ...
  let mockEditCommit;
  beforeEach(() => {
    let androidPublisherObj = new androidpublisher_v3.Androidpublisher({
      auth: jwtObjOriginal
    });
    mockEditInsert = jest
      .spyOn(androidPublisherObj.edits, "insert")
      .mockImplementation(data => {
        ...
        return Promise.resolve({
          data: {
            id: "123456",
            expiryTimeSeconds: new Date().getMilliseconds()
          }
        });
      });
    mockEditApkUpload = jest
      .spyOn(androidPublisherObj.edits.apks, "upload")
      .mockImplementation(() => {
        return Promise.resolve({
          data: {
            versionCode: "10.0",
    ...
    mockEditTrackUpdate = jest
      .spyOn(androidPublisherObj.edits.tracks, "update")
      .mockImplementation(() => {
        return Promise.resolve({
          data: {
            release: "10.0",
    ...
    mockEditCommit = jest
      .spyOn(androidPublisherObj.edits, "commit")
      .mockImplementation(() => {
        return Promise.resolve({
          data: {
  ...
  it("Edit - method in upload class when packageName missing", async () => {
    ...
    try {
      ...
      expect(e.message).toBe(
        "Error while getting edit details packageName not found"
      );
    }
  });
  ...
  it("Function: commitRelease when track is undefined", async () => {
    try {
      ...
      expect(e.message).toBe(
        "Error while upload files, can not find track details."
      );
    }
  });
