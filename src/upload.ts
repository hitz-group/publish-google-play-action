...
export class upload {
  ...
  async uploadRelease(): Promise<AppEdit> {
    ...
    console.log(
      `apk was uploaded to play store with commit id ${commitStatus.id}`
    );
  ...
  async commitRelease(track: Track | undefined): Promise<AppEdit> {
    if (track != undefined) {
      ...
      console.log(
        `Committed release with Id(${res.data.id}) and Track: ${track.track}.`
      );
  ...
  async uploadApk(): Promise<Apk> {
    console.log(
      `[${this.appEditId}, packageName=${this.apkManifest.packageName}]: Uploading APK @ ${this.releaseFile}`
    );
  ...
  async trackVersionCode(): Promise<Track> {
    console.log(
      `Creating Track Release for Edit(${this.appEditId}) for Track(${this.editOptions.track}) and VersionCode(${this.versionCode})`
    );
